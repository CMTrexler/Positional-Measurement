...
if isequal(folderPath, 0)
    ...
    for k = 1:length(cellArray_averagedFrames)
        ...
        if  isempty(max(widths))
            ...
            widthDatapointForAvg1  = max(widths); % Simply saving the data point temporarily while efficacy is determined for finding average peak width.
                                                 %      Notably this is not a universal solution as interference patterns typically have fringe widths
                                                 %      that are smaller the futher they are away from the center of the beam-spot. So this may only work for
                                                 %      determininingfringe efficacy ina small region around the first peak
            ...
            widthDatapointForAvg2 = max(widths);
            ...
            if gof1.rsquare < 0.7 || gof2.rsquare < 0.7
                ...
                widthAveragesArray = [widthAveragesArray, widthDatapointForAvg1, widthDatapointForAvg2];
            end
        end
    end
    fringeSpacingAvg = sum(fringeSpacingArray) / length(fringeSpacingArray);
    widthAverageSize = sum(widthAveragesArray) / numel(widthAveragesArray);
    ...
    for k = 2:length(cellArray_averagedFrames)
        ...
        % Determine efficacy of frame. Prominence will be used to check,
        %   presence of only low prominence peaks means noisy frame
        if  isempty(max(newFringeWidths)) || max(newFringeWidths) < (widthAverageSize * 0.85) || max(newFringeWidths) > (widthAverageSize * 1.15)
            % Frame is too noisy, skip
            fprintf('Frame %d is too noisy and was skipped. \n', k)
